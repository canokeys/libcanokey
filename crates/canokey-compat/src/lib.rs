//! Immutable CanoKey device profiles, capability evidence, and firmware rules.
//!
...
//! audited firmware matrix covering releases 1.3 through 3.1.0. Every answer
//! is a [`CapabilityStatus`] pairing a three-valued [`Support`] decision with
//! its [`Evidence`] provenance; `Unknown` is deliberately distinct from
//! `Unsupported`. It also defines semantic [`Algorithm`] identifiers,
//! firmware-version parsing ([`FirmwareVersion`]), and PIV wire-ID mappings.
//! The crate performs no device I/O: probing a card is the facade's job.
//!
//! Most applications should depend on the `canokey` facade crate, which probes
//! a device and builds a profile for you. Depend on `canokey-compat` directly
//! when you construct a [`DeviceProfile`] manually from observations you
//! obtained yourself, or when you need to inspect capability evidence without
//! the facade.
//!
//! The full compatibility model lives in `docs/design/api-design.md`
//! (section "Profiles and probing") in the repository.
#![deny(missing_docs)]
#![forbid(unsafe_code)]
